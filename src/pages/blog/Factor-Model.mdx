...
First, I'll share what a factor model is and then I'll circle back to how it connects to synthetic controls.
...
Under a factor model, the synthetic control unit is formed as the weighted average of the control unit's outcomes $\hat{y}_{it}^{sc} = \sum_j w_j y_{jt}$, where $w_j$ are found via the synthetic control procedure. Ignoring the error term, we can write this as
$$
...
When does the synthetic control do a good job? Well, when the synthetic control units' factor-loadings $\bm{\gamma}^{sc}$ look similar to the treated units factor-loadings! If we satisfy this condition, then we are confident in our ability to predict the counterfactual outcome in the post-periods (since the synthetic control and the treated unit are subject to the same factor shocks and they have the same "exposure" to those shocks.)

I find this interpretation really useful. As a researcher, I can start thinking of these conditions a-priori. First, I think of factors that are important in my setting (e.g. in the GDP setting, important factors include a counties' industrial mix.). Second, I think of whether my treated units' factor-loadings (e.g. their industrial mix) can be well approximated by the donor pool. This would fail in our ongoing example if my treated county is an outlier in terms of industrial mix. 

The last connection I'll make is about the importance of long panels for estimating synthetic control. The importance of long panels is to ensure that the weights selected, $w_j$, do a good job at approximating the treated units' factor-loadings. In short-panels, the synthetic control weights can be affected by noise and do a bad job at identifying the underlying factor structure in the data. 

## Examples of factor-models in synthetic control papers

I'll end with a few examples of factor models in synthetic control papers.

1. [Ferman and Pinto - Synthetic Control with Imperfect Pre-Treatment Fit](https://arxiv.org/pdf/1911.08521.pdf)

Considers what happens under a factor model when the pre-treatment fit is imperfect.
...
2. [Ben-Michael, Feller, and Rothstein - The Augmented Synthetic Control Method](https://arxiv.org/pdf/1811.04170.pdf)

Considers how to use regularization on the weights to prevent over-fitting of the weights in shorter panels. This helps better approximate the true treated unit's factor-loadings in a factor-model.

3. [Gobillon and Magnac - Regional Policy Evaluation: Interactive Fixed Effects and Synthetic Controls](https://direct.mit.edu/rest/article-abstract/98/3/535/58348/Regional-Policy-Evaluation-Interactive-Fixed) and [Xu - Generalized Synthetic Control Method](https://yiqingxu.org/papers/english/2016_Xu_gsynth/Xu_PA_2017.pdf)
...
<Figure  src="../images/blog/blog-factor-many_estimators.png" alt="Example of plug-in estimator for the effect of Walmart on local employment" />
